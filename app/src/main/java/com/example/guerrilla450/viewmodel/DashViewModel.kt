...
import com.example.guerrilla450.dash.DashKeepAliveService
import com.example.guerrilla450.dash.DashSession
import com.example.guerrilla450.dash.DashState
import com.example.guerrilla450.dash.DashWifiManager
import com.example.guerrilla450.dash.WifiConnStatus
import com.example.guerrilla450.dash.map.LocationTracker
import com.example.guerrilla450.dash.map.MapRenderer
import com.example.guerrilla450.dash.map.Mercator
import com.example.guerrilla450.dash.map.TileProvider
import com.example.guerrilla450.dash.nav.GeoPoint
import com.example.guerrilla450.dash.nav.Route
import com.example.guerrilla450.dash.nav.Router
import com.example.guerrilla450.dash.protocol.DashCommands
import com.example.guerrilla450.dash.video.DashEncoder
import com.example.guerrilla450.dash.video.NalProcessor
import com.example.guerrilla450.dash.video.RtpPacketizer
...
class DashViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private val dashConfig  = com.example.guerrilla450.dash.DashConfig.get(app)
    private val voice        = com.example.guerrilla450.dash.nav.VoiceManager.get(app)
    private val repo         = com.example.guerrilla450.data.SyncRepository.get(app)
    private val recorder     = com.example.guerrilla450.data.RideRecorder()
    ...
    companion object {
        ...
        // and 45 s wasn't enough to safely pull over + restart before Guerrilla 450 gave up. With the
    ...
    init {
        ...
        viewModelScope.launch {
            wifiManager.state.collect { ws ->
                com.example.guerrilla450.data.RideDiagnostics.log(
                    "wifi",
                    ws.status.toString() +
                ...
                when (ws.status) {
                    ...
                    WifiConnStatus.ERROR -> {
                        ...
                        com.example.guerrilla450.data.RideDiagnostics.stop("wifi error / dash gone")
                    }
                    else -> refreshStage()
                }
            }
        ...
        viewModelScope.launch {
            session.state.collect { state ->
                com.example.guerrilla450.data.RideDiagnostics.log("session", "→ $state")
                refreshStage()
                ...
                when (state) {
                    ...
                    DashState.STREAMING -> {
                        ...
                        com.example.guerrilla450.util.Telemetry.logEvent("dash_streaming")
                    }
        ...
        session.onError = { msg ->
            com.example.guerrilla450.data.RideDiagnostics.log("error", msg)
        ...
        session.onButton = { btn ->
            ...
            val call = com.example.guerrilla450.media.CallInfoProvider.incomingCall.value
            ...
            com.example.guerrilla450.data.RideDiagnostics.log("button",
    ...
    fun connect() {
        ...
        com.example.guerrilla450.data.RideDiagnostics.init(getApplication())
        com.example.guerrilla450.data.RideDiagnostics.start("connect")
        ...
        com.example.guerrilla450.data.RideDiagnostics.log(
            "build", "apk=${com.example.guerrilla450.util.BuildId.sha12(getApplication())}" +
                " vc=${com.example.guerrilla450.BuildConfig.VERSION_CODE} name=${com.example.guerrilla450.BuildConfig.VERSION_NAME}",
        )
        com.example.guerrilla450.data.RideDiagnostics.log(
    ...
    fun disconnect() {
        ...
        com.example.guerrilla450.data.RideDiagnostics.log("connect", "user disconnect")
        stopRecording()        // a connect→disconnect session = one saved ride
        session.disconnect()
        wifiManager.disconnect()
        releaseBackgroundResources()
        refreshStage()
        com.example.guerrilla450.data.RideDiagnostics.stop("disconnect")
        ...
        com.example.guerrilla450.data.DiagnosticsUploader.uploadPending(getApplication())
    }
    ...
    private fun giveUp(reason: String) {
        ...
        releaseBackgroundResources()
        refreshStage()
        com.example.guerrilla450.data.RideDiagnostics.log("error", "gave up: $reason — background released")
        com.example.guerrilla450.data.RideDiagnostics.stop(reason)
    }
    ...
    private fun armConnLostAlert() {
        ...
        connLostAlertJob = viewModelScope.launch {
            ...
            if (session.state.value != DashState.STREAMING && userWantsConnection) {
                ...
                com.example.guerrilla450.data.RideDiagnostics.log("connect", "sustained link loss → rider alert (vibrate + voice)")
                runCatching {
    ...
    fun exitNavigation() {
        ...
        session.updateRouteCard("Guerrilla 450")   // dash card → name + 0.0 km, nav off
    }
    ...
    private fun startStream() {
        com.example.guerrilla450.data.RideDiagnostics.log("stream", "startStream — encoder up, RTP→dash beginning")
        videoPtsMs = 0L
        ...
        val onEncoded: (ByteArray, Boolean) -> Unit = { annexB, isKey ->
            ...
            if (!loggedFirstFrame) {
                ...
                com.example.guerrilla450.data.RideDiagnostics.log("stream", "first video frame sent (key=$isKey, ${annexB.size}B)")
            }
    ...
    private fun tick(): Boolean {
        ...
        val sig = buildString {
            ...
            append(com.example.guerrilla450.media.CallInfoProvider.incomingCall.value?.takeIf { it.incoming }?.caller ?: "")
        }
    ...
    private fun maybeReroute(offRoute: Boolean, loc: android.location.Location?) {
        ...
        com.example.guerrilla450.data.RideDiagnostics.log("reroute", "off-route ${offSec}s → requesting new route")
        ...
        viewModelScope.launch {
            ...
            if (r != null) {
                ...
                com.example.guerrilla450.data.RideDiagnostics.log("reroute", "new route in ${took}ms (${r.geometry.size} pts, ${r.totalMeters.toInt()} m)")
            } else {
                android.util.Log.w("DashViewModel", "Reroute failed (no internet?)")
                com.example.guerrilla450.data.RideDiagnostics.log("reroute", "FAILED after ${took}ms (no internet?)")
            }
            rerouting = false
        }
    }
    ...
    private fun answerCall(c: com.example.guerrilla450.media.IncomingCall) {
        val sent = c.answerIntent?.let { runCatching { it.send() }.isSuccess } ?: false
        if (!sent) callController.answer()
        com.example.guerrilla450.data.RideDiagnostics.log("media", "answer (notifAction=$sent)")
    }

    /** Reject (ringing) or hang up (active) via the dialer's action; fall back to TelecomManager. */
    private fun endCall(c: com.example.guerrilla450.media.IncomingCall) {
        val sent = c.declineIntent?.let { runCatching { it.send() }.isSuccess } ?: false
        if (!sent) callController.hangup()
        com.example.guerrilla450.data.RideDiagnostics.log("media", "end/reject (notifAction=$sent)")
    }
    ...
    private data class NavState(
        ...
        val nextManeuver: com.example.guerrilla450.dash.nav.Maneuver?,
    )
    ...
    private fun trackProgress(r: Route, pos: GeoPoint): NavState {
        ...
        val nextMan = r.maneuvers.firstOrNull {
            it.cumulativeMeters > progressM + 1.0 && it.type != com.example.guerrilla450.dash.nav.ManeuverType.DEPART
        }
    ...
    private fun startMediaForwarding() {
        val granted = com.example.guerrilla450.media.MediaInfoProvider.isAccessGranted(getApplication())
        com.example.guerrilla450.data.RideDiagnostics.log("media", "forwarding start — notification access=$granted")
        mediaInfo.start()
        mediaObserveJob?.cancel()
        mediaObserveJob = viewModelScope.launch {
            launch {
                mediaInfo.nowPlaying.collect { np ->
                    com.example.guerrilla450.data.RideDiagnostics.log("media", "now-playing observed: ${np?.title ?: "(none)"}")
                    session.updateNowPlaying(np?.title, np?.album.orEmpty(), np?.artist.orEmpty())
                }
            }
            launch {
                com.example.guerrilla450.media.CallInfoProvider.incomingCall.collect { call ->
                    com.example.guerrilla450.data.RideDiagnostics.log("media", "call observed: ${call?.caller ?: "(none)"}")
    ...
    override fun onCleared() {
        ...
        releaseBackgroundResources()
        voice.shutdown()
        com.example.guerrilla450.data.RideDiagnostics.stop("app closed")
    }
}
