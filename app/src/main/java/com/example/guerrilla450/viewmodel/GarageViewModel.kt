...
class GarageViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private fun reload() = viewModelScope.launch {
        ...
        withContext(Dispatchers.IO) {
            com.example.guerrilla450.data.MaintenanceNotifier.check(getApplication(), ui.maint.map { it.item }, ui.odometerKm.toInt())
            com.example.guerrilla450.data.MaintenanceNotifier.checkDocuments(getApplication(), ui.documents)
        }
    }
    ...
    fun saveDocument(
        ...
        withContext(Dispatchers.IO) {
            ...
            repo.upsertDocument(VehicleDocument(
                sid = sid ?: GuerrillaDb.newSid(), type = type, title = title, number = number,
