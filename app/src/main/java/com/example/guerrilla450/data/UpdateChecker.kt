...
object UpdateChecker {
    ...
    private const val LATEST_RELEASE_API =
        "https://api.github.com/repos/harshith001/Guerrilla450/releases/latest"
    private const val RELEASES_PAGE =
        "https://github.com/harshith001/Guerrilla450/releases/latest"
     ...
     * how it got there. Mirrors the test channel ([com.example.guerrilla450.data.TestBuildChecker]).
     */
    fun shouldOffer(context: Context, remote: ReleaseInfo): Boolean {
        val mine = com.example.guerrilla450.util.BuildId.sha12(context)
    ...
    fun fetchLatest(context: Context): ReleaseInfo? = runCatching {
        val conn = (URL(LATEST_RELEASE_API).openConnection() as HttpURLConnection).apply {
            ...
            setRequestProperty("User-Agent", "Guerrilla450-App")
    ...
    fun download(context: Context, url: String, onProgress: (Float) -> Unit): File? = runCatching {
        ...
        val out = File(dir, "guerrilla-update.apk")
